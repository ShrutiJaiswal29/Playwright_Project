...
```ts
  ...
  20 |     const newAccountId = await account.createAccount('SAVINGS')
  21 |     console.log('Account id created:', newAccountId)
  22 | 
  23 |     const response = await request.get(`${baseUrl}/accounts/${newAccountId}`, {headers: {Accept: 'application/json'}})
  24 |     console.log('API status:', response.status())
> 25 |     const body = await response.json()
     |                  ^ SyntaxError: Unexpected end of JSON input
  26 |     console.log('API response:', body)
  27 | 
  28 |     expect(response.status()).toBe(200)
  29 |     expect(body.id.toString()).toBe(newAccountId)
  30 |     expect(body.type).toBe('SAVINGS')
  31 |     expect(typeof body.balance).toBe('number')
  32 |     console.log('HYBRID TEST PASSED')
  33 | })
  34 | 
```